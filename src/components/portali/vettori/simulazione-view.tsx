...
export function SimulazioneView() {
  ...
  // L'addebito al cliente parte da quello calcolato per il vettore scelto, ma
  // lo decide l'operatore: e' il dato che poi arriva sulla bolla.
  useEffect(() => {
    const importo = scelto?.riaddebito?.importo;
    setAddebito(importo === null || importo === undefined ? "" : importo.toFixed(2).replace(".", ","));
  }, [scelto?.vettoreId, scelto?.riaddebito?.importo]);
  ...
  async function salvaBolla() {
    ...
    const addebitoCliente = addebito.trim() ? Number(addebito.trim().replace(",", ".")) : null;
    if (addebitoCliente !== null && !(Number.isFinite(addebitoCliente) && addebitoCliente >= 0)) { setErrore("L'addebito al cliente deve essere un importo in euro, anche zero."); return }
    setSalvataggio(true); setErrore(null);
    try {
      const res = await fetch("/api/portali/vettori/simulazioni", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ simulazione: { ...datiInviati, vettoreSceltoId: scelto.vettoreId, costoPrevisto: scelto.calcolo.totale, riaddebitoPrevisto: addebitoCliente, esiti: risposta?.risultati ?? [] }, bolla: { numeroRiferimento: numeroDopo ? null : numeroBolla.trim(), dataDocumento, controparteNome: controparte.trim(), controparteCodice: codiceCliente.trim() || null } }) });
    ...
    <div className="mx-auto max-w-[1380px] pb-12 text-text selection:bg-primary/20">
      ...
      <div className="grid items-start gap-7 xl:grid-cols-[minmax(0,1.05fr)_minmax(28rem,0.95fr)]">
        ...
        <div className="space-y-4 xl:sticky xl:top-4">
          ...
          {scelto ? <Conferma scelto={scelto} aperta={confermaAperta} setAperta={setConfermaAperta} numero={numeroBolla} setNumero={setNumeroBolla} dopo={numeroDopo} setDopo={setNumeroDopo} data={dataDocumento} setData={setDataDocumento} controparte={controparte} setControparte={setControparte} addebito={addebito} setAddebito={setAddebito} salvataggio={salvataggio} salva={salvaBolla} /> : null}
