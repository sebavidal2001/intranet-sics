-- 069_lavorazioni_divideq_e_codice_commessa.sql
--
-- Due modifiche alle RPC del builder (basate sulla definizione corrente, migration 065):
--
-- 1) [LAVORAZIONI ÷Q] Il moltiplicatore dei servizi con scala_con_quantita = true
--    passa da × Q a ÷ Q: il costo delle lavorazioni è inserito per l'INTERO lotto e
--    va RIPARTITO sul singolo pezzo. Le voci una-tantum (scala = false, es.
--    Progettazione/Manuale) restano × 1. Uso 1.0/v_q (numeric) per evitare la
--    divisione intera. v_q è già GREATEST(1, ...) quindi mai zero.
--
-- 2) [CODICE COMMESSA] `crea_documento_dal_builder` NON genera più il progressivo
--    interno G (next_codice_generato): il codice è obbligatorio e arriva dal builder
--    (codice commessa inserito dall'utente). Se manca → errore. I doppioni sono
--    bloccati dal vincolo UNIQUE su documenti.codice (23505 → 409 lato route).
--    La sequence codice_g_seq e la funzione next_codice_generato restano nel DB,
--    inutilizzate (verranno rimosse quando si ripuliranno i vecchi codici G).
--
-- `aggiorna_documento_dal_builder` NON tocca il codice (lo preserva dal record),
-- quindi qui cambia solo il ÷Q.
--
-- I preventivi ESISTENTI non si ricalcolano: i totali sono materializzati in
-- blocchi/righe_distinta. Il nuovo calcolo vale per i preventivi nuovi e per le
-- bozze che vengono riaperte e RISALVATE.

-- ── crea_documento_dal_builder: ÷Q + codice commessa obbligatorio ─────────────
    ...
    BEGIN
      ...
      IF blocco ? 'servizi' THEN
        ...
        LOOP
          ...
          v_mult  := CASE WHEN v_scala THEN 1.0 / v_q ELSE 1 END;   -- ÷Q se scala (ripartita), altrimenti ×1
...
-- ── aggiorna_documento_dal_builder: solo ÷Q (codice preservato dal record) ─────
