...
import { z } from "zod";
import { modificaKpi } from "../../../actions";
...
export default function ModificaKpiForm({ kpi, parametri }: { kpi: KpiRow; parametri: Parametro[] }) {
  ...
  const { register, handleSubmit, formState: { errors } } = useForm<FormValues>({
    ...
    defaultValues: {
      nome: kpi.nome,
      parametro_id: kpi.parametro_id ?? "",
      operatore: kpi.operatore,
      soglia: kpi.soglia,
      anno: kpi.anno ?? (0 as unknown as undefined),
    },
  });
  ...
  const onSubmit = (values: FormValues) => {
    ...
    startTransition(async () => {
      ...
      const result = await modificaKpi(kpi.id, fd);
  ...
  return (
    <form onSubmit={handleSubmit(onSubmit)} className="space-y-5">
      ...
      <div className="space-y-1.5">
        <label className="block font-tenorite text-sm text-text">Nome <span className="text-danger">*</span></label>
      ...
      <div className="grid grid-cols-2 gap-4">
        <div className="space-y-1.5">
          <label className="block font-tenorite text-sm text-text">Operatore <span className="text-danger">*</span></label>
        ...
        <div className="space-y-1.5">
          <label className="block font-tenorite text-sm text-text">Soglia <span className="text-danger">*</span></label>
      ...
      <div className="space-y-1.5">
        ...
        <input {...register("anno")} type="number" min="2020" max="2050" placeholder="Tutti gli anni" className={inputClass} />
      </div>
      ...
      <div className="flex gap-3 pt-2">
        ...
        <Link href="/admin/config" className="px-5 py-2.5 rounded-lg text-sm border border-border text-text-muted hover:text-text hover:border-text-muted transition-colors">
          Annulla
        </Link>
      </div>
    </form>
