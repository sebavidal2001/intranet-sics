...
import { z } from "zod";
import { modificaScala } from "../../../actions";
...
export default function ModificaScalaForm({ scala }: { scala: Scala }) {
  ...
  const min = useWatch({ control, name: "min" }) || scala.min;
  const max = useWatch({ control, name: "max" }) || scala.max;

  const [labels, setLabels] = useState<Record<string, string>>(scala.labels ?? {});
  ...
  useEffect(() => {
    ...
    if (!isNaN(minN) && !isNaN(maxN) && minN <= maxN) {
      ...
      for (let i = minN; i <= maxN; i++) {
        next[String(i)] = labels[String(i)] || scala.labels?.[String(i)] || "";
      }
      setLabels(next);
    }
  ...
  const onSubmit = (values: FormValues) => {
    ...
    startTransition(async () => {
      ...
      const result = await modificaScala(scala.id, fd);
    ...
    <form onSubmit={handleSubmit(onSubmit)} className="space-y-5">
      ...
      <div className="grid grid-cols-2 gap-4">
        <div className="space-y-1.5">
          ...
          <input {...register("min")} type="number" min={1} className={inputClass} />
        ...
        <div className="space-y-1.5">
          ...
          <input {...register("max")} type="number" max={100} className={inputClass} />
