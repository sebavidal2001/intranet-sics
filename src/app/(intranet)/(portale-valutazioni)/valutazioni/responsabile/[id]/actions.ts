...
export async function salvaRisposteResponsabile(
  data: SalvaRisposteData
  ...
  if (sessione.responsabile_id !== user.id) return { error: "Non autorizzato" };
  ...
  if (data.completa) {
    if (sessione.stato !== "resp_in_corso") {
      return { error: "La sessione non è in uno stato che consente questa operazione" };
    }
    // Usa admin client per bypassare RLS sull'update dello stato
    const adminClient = createAdminClient();
    const { error: errUpdate } = await adminClient
      .from("sessioni_utente")
      .update({ stato: "collab_in_corso" })
      .eq("id", data.sessione_id);

    if (errUpdate) return { error: errUpdate.message };
  }

  revalidatePath(`/valutazioni/responsabile/${data.sessione_id}`);
