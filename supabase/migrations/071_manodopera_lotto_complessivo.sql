-- 071_manodopera_lotto_complessivo.sql
--
-- [CORREZIONE MODELLO ÷Q] Coerenza tra vista unitaria e totale complessivo.
--
-- La migration 069 aveva introdotto il ÷Q applicandolo al TOTALE COMPLESSIVO del
-- blocco (`v_mult := 1.0 / v_q`). Conseguenza: la manodopera veniva divisa per Q
-- anche nel complessivo, quindi il totale sottostimava le ore di Q volte e non
-- valeva più `unitario × Q = complessivo`.
--
-- Semantica corretta (concordata):
--   * le ore di manodopera si riferiscono all'INTERO LOTTO → nel totale
--     COMPLESSIVO contano UNA VOLTA (× 1), qualunque sia `scala_con_quantita`.
--     Es. 1000 € di lavorazione con 10 pezzi → 1000 € nel totale (non 10.000, non 100).
--   * i materiali restano inseriti per 1 pezzo → × Q nel complessivo.
--   * la vista PER PEZZO è calcolata lato UI e lì il flag CONTA:
--       - `scala_con_quantita = true`  (÷Q) → 1000 € / 10 = 100 € per pezzo
--       - `scala_con_quantita = false` (1×) → 1000 € interi anche sul pezzo (una tantum)
--     Con voci "1×" l'unitario NON è il complessivo ÷ Q: sono due viste distinte.
--
-- Quindi qui `v_mult` torna a 1 per i servizi: il DB materializza solo i totali
-- complessivi, mentre la ripartizione per pezzo vive nel frontend.
--
-- I preventivi ESISTENTI non si ricalcolano (i totali sono materializzati in
-- blocchi/righe_distinta): la nuova formula vale per i nuovi salvataggi e per le
-- bozze riaperte e risalvate.

-- ── crea_documento_dal_builder ────────────────────────────────────────────────
...
BEGIN
  ...
  LOOP
    DECLARE
      ...
      v_mult       numeric := 1;   -- manodopera: ore dell'intero lotto → × 1
    BEGIN
...
-- ── aggiorna_documento_dal_builder ────────────────────────────────────────────
