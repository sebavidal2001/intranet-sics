-- Migration 047: cost model v3 — allinea al modello canonico degli Excel SICS.
-- Novità rispetto a v2 (046):
--   * Aggiunge TEMPI ACCESSORI DI PRODUZIONE = 2,8% × (materiale_COSTO + manodopera_COSTO)
--   * SPESE GENERALI ora = 24,2% × (materiale_COSTO + manodopera_COSTO)   [prima: sul prezzo]
--   * IMBALLAGGIO resta 1% × (materiale_VENDITA + manodopera_VENDITA)     [sul prezzo]
--   prezzo_finale = (base_vendita + imballaggio + tempi + spese) × (1 + margine%)
--
-- Le quantità: materiali ×Q sempre; manodopera ×Q se scala_con_quantita, altrimenti ×1
-- (una tantum). La presentazione "ammortizzata /Q" è solo lato UI; qui i totali sono complessivi.

ALTER TABLE preventivatore.blocchi
  ADD COLUMN IF NOT EXISTS tempi_accessori_pct numeric(6,3) NOT NULL DEFAULT 2.8;
...
BEGIN
  ...
  LOOP
    ...
    BEGIN
      ...
      v_tot := v_tot + (
        (v_base_vend
          + v_base_vend * (v_imb_pct/100)
          + v_costo_blk * (v_tempi_pct/100)
          + v_costo_blk * (v_spese_pct/100)
        ) * (1 + v_marg_eff/100)
      );
        ...
        VALUES
          (v_doc_id, 'builder', cod_block, art->>'codice', art->>'descrizione',
           ...
           CASE WHEN (art->>'coeff_ricarico')::numeric > 0
                THEN (art->>'ult_costo')::numeric * (art->>'qty')::numeric / (art->>'coeff_ricarico')::numeric ELSE 0 END,
           CASE WHEN (art->>'coeff_ricarico')::numeric > 0
                THEN ceil((art->>'ult_costo')::numeric * (art->>'qty')::numeric / (art->>'coeff_ricarico')::numeric * 100) / 100 ELSE 0 END,
           'materiale', true);
      END LOOP;
      IF blocco ? 'servizi' THEN
        FOR srv IN SELECT * FROM jsonb_array_elements(blocco->'servizi')
          ...
          VALUES
            (v_doc_id, 'builder', cod_block, NULL, srv->>'nome',
             ...
             CASE WHEN (srv->>'coeff_ricarico')::numeric > 0
                  THEN (srv->>'tariffa_ora')::numeric * (srv->>'ore')::numeric / (srv->>'coeff_ricarico')::numeric ELSE 0 END,
             CASE WHEN (srv->>'coeff_ricarico')::numeric > 0
                  THEN ceil((srv->>'tariffa_ora')::numeric * (srv->>'ore')::numeric / (srv->>'coeff_ricarico')::numeric * 100) / 100 ELSE 0 END,
